...
vec3 calculateDirectIllumiunation(vec3 wo, vec3 n, vec3 base_color)
{
	...
	//            return vec3(0);
	///////////////////////////////////////////////////////////////////////////
	const vec3 light = viewSpaceLightPosition - viewSpacePosition;
	const float light_distance = length(light);
	const float factor = 1.0 / (light_distance * light_distance);
	const vec3 wi = normalize(light);
	const vec3 Li = point_light_color * point_light_intensity_multiplier * factor;
	if(dot(wi, n) <= 0.0)
	    return vec3(0.0);
...
vec3 apMotionBlur(vec2 coord)
{
    const int numSamples = 10;     
    const float blurScale = 0.2;  

    vec2 blurDirection = screenVelocity * blurScale / float(numSamples);
    vec3 accumulatedColor = vec3(0.0);
    float totalWeight = 0.0;

    for (int i = -numSamples / 2; i <= numSamples / 2; i++) {
        vec2 offsetCoord = coord + blurDirection * float(i);
        accumulatedColor += texture(colorMap, offsetCoord).rgb;
        totalWeight += 1.0;
    }
